...
<p align="center">
  <a href="#product-philosophy">Philosophy</a> · <a href="#before-you-ab-test">Before A/B test</a> · <a href="#what-you-can-experiment-with">Experiments</a> · <a href="#design-decisions">Design decisions</a> · <a href="#features">Features</a> · <a href="#getting-started">Getting started</a> · <a href="#built-with-claude">Built with Claude</a> · <a href="#impact--support">Impact</a>
</p>
...
But this is not just about efficiency. It is about **responsibility**.

Every email you send lands in someone's personal space. Their inbox. Their lock screen. Their morning. "Just A/B test it" is not a strategy — it is an abdication of craft. It says: "I don't know if this is good, so let me send both versions to thousands of real people and see which one is less bad." Those are real people. They gave you their email address. That is trust.

If your subject line is lazy, misleading, or indistinguishable from spam, you are not "testing" — you are wasting someone's attention. Attention is finite and non-renewable. Every bad email trains the recipient to ignore you. The unsubscribe is not a metric failure. It is the recipient telling you: *you broke my trust*.

SubjectLab exists because **the words you send to people matter**. Before you test, look at what you're about to send. See it the way they will see it. If it doesn't survive the inbox — if you can't even find it yourself in 3 seconds — it has no business being in anyone's inbox.

Craft the message. Then test it. Not the other way around.
...
しかしこれは効率の話だけではない。**責任**の話だ。

あなたが送る全てのメールは、誰かのパーソナルスペースに着地する。受信トレイ。ロック画面。朝の時間。「とりあえずA/Bテストしとけばいい」は戦略ではない——それはクラフトの放棄だ。「これが良いか分からないから、両方のバージョンを何千人もの実在の人間に送って、どちらがマシか見よう」と言っているのと同じだ。その何千人は実在の人間だ。あなたにメールアドレスを渡した。それは信頼だ。

件名が手抜きで、誤解を招き、スパムと見分けがつかないなら、あなたは「テスト」しているのではない——誰かの注意を浪費している。注意は有限で、再生しない。ダメなメールの一通一通が、受信者にあなたを無視することを学習させる。配信停止はメトリクスの失敗ではない。受信者からのメッセージだ：*信頼を裏切った*。

SubjectLabが存在するのは、**人に届ける言葉には責任がある**からだ。テストする前に、自分がこれから送ろうとしているものを見ろ。受信者が見るように見ろ。受信トレイで生き残れないなら——3秒で自分のメールすら見つけられないなら——それは誰の受信トレイにも存在すべきではない。
...
### The linguistics of the inbox · 受信トレイの言語学
...
**Linearity (線条性)** — The signifier unfolds in a one-dimensional chain. The device truncates this chain at a physical boundary. SubjectLab tracks not just *how many characters* survive, but *which tokens* — highlighting the exact word where the signifier breaks.

**シニフィアンの線条性** — シニフィアンは一次元の連鎖として展開する。デバイスはこの連鎖を物理的境界で切断する。SubjectLabは「何文字生存するか」だけでなく「どのトークンが」を追跡し、シニフィアンが壊れる正確な語をハイライトする。

**Syntagmatic relations (連辞関係)** — Sender → Subject → Preview form a syntagmatic chain. Each element should advance meaning, not repeat it. SubjectLab auto-detects shared morphemes between sender and subject, and measures lexical overlap between subject and preview.

**連辞関係** — 送信者名→件名→プレビューは連辞的連鎖を形成する。各要素は意味を前進させるべきで、繰り返すべきではない。SubjectLabは送信者名と件名の共有形態素を自動検出し、件名とプレビューの語彙重複率を計測する。

**Paradigmatic relations (範列関係)** — The inbox is a visible paradigm: a vertical column of substitutable signs. Your email must possess a *trait distinctif*. SubjectLab scans for shared opening patterns, lead phrases, and emoji positions among competing emails.
...
|---|---|---|
| Linearity / 線条性 | Character + word-level truncation across 7 environments | ✅ |
| Key signifier position / キーシニフィアン | Broken token highlighted in amber, lost tokens in strikethrough | ✅ |
| Syntagmatic chain / 連辞的連鎖 | Sender → Subject → Preview as unified sequence | ✅ |
| Syntagmatic redundancy / 連辞的冗長 | Auto-detect shared morphemes + lexical overlap | ✅ |
| Paradigmatic contrast / 範列的対比 | Inbox Simulator with industry competitors | ✅ |
| Paradigmatic test / 範列的テスト | 3-Second Challenge: timed findability | ✅ |
| Paradigmatic similarity / 範列的類似 | Auto-scan for shared patterns among competitors | ✅ |

---

### Beyond the inbox: notification as the true first screen · 受信トレイの先へ：通知こそ真のファーストスクリーン
...
### What SubjectLab does NOT address · SubjectLabが扱わないもの
...
### Signifier survival experiments · シニフィアン生存実験
...
### Syntagmatic chain experiments · 連辞的連鎖の実験
...
### Paradigmatic distinctiveness experiments · 範列的弁別性の実験
...
### Cognitive triage experiments · 認知トリアージの実験
...
### 1 — Simulation over scoring · スコアリングより視覚シミュレーション
...
### 2 — Industry context matters · 業界コンテキストが重要
...
### 3 — Position control with a slider · スライダーによる位置制御
...
### 4 — Notification screen as first impression · 通知画面がファーストインプレッション
...
### 5 — AI is optional, not core · AIはオプション
...
### 6 — Desktop only, intentionally · デスクトップ専用は意図的
...
### 7 — Export as communication tool · エクスポートはコミュニケーション
...
### 8 — Curated realism over generation · 生成よりキュレーションされたリアリズム
...
### Has SubjectLab improved your KPIs? · SubjectLabでKPIが改善しましたか？
...
**Share your results through any channel:**

- **GitHub Discussions** — Post in the [Success Stories](https://github.com/ysnotksk/subject-lab/discussions/categories/success-stories) category. Structured reports help the community learn. Include before/after metrics if you can.
- **X (Twitter)** — Mention [@ysnotksk](https://x.com/ysnotksk) or use `#SubjectLab`. Public sharing spreads the word and helps others discover the tool.
- **LinkedIn** — [linkedin.com/in/ysnotksk](https://linkedin.com/in/ysnotksk)
- **GitHub** — [github.com/ysnotksk](https://github.com/ysnotksk)

**成果を共有してください：**

- **GitHub Discussions** — [Success Stories](https://github.com/ysnotksk/subject-lab/discussions/categories/success-stories)カテゴリに投稿。構造化されたレポートがコミュニティの学びになります。可能なら改善前後の指標を含めてください。
- **X (Twitter)** — [@ysnotksk](https://x.com/ysnotksk)にメンション、または`#SubjectLab`を使用。公開共有はツールの認知を広げます。
- **LinkedIn** — [linkedin.com/in/ysnotksk](https://linkedin.com/in/ysnotksk)
- **GitHub** — [github.com/ysnotksk](https://github.com/ysnotksk)
...
- **GitHub Sponsors** — [github.com/sponsors/ysnotksk](https://github.com/sponsors/ysnotksk)
- **コード貢献** — IssueとPRは常に歓迎
...
I'm open to new opportunities. If you're building something where product thinking, linguistic intelligence, and frontend craft matter — or if your product marketing isn't landing and you can't figure out why — let's connect on any of the channels above.
...
新しい機会を探しています。プロダクト思考、言語学的知性、フロントエンドクラフトが重要なものを作っているなら——あるいはプロダクトマーケティングがうまくいかない理由が分からず困っているなら——上記いずれかのチャネルでご連絡ください。
